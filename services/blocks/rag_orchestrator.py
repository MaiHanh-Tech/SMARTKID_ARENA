...
class TranslationOrchestrator:
    """
    Translation orchestrator - Tích hợp vào RAG orchestrator
    
    Chức năng:
    - Dịch văn bản với progress tracking
    ...
    def translate_document(
        ...
        """
        Dịch văn bản (Standard mode)
        
        Args:
            input_text: Văn bản cần dịch
            source_lang: Ngôn ngữ nguồn
            target_lang: Ngôn ngữ đích
            include_english: Có kèm English không
            progress_callback: Hàm callback (0.0-1.0)
...
{input_text}"""
    ...
    def translate_interactive(
        ...
        """
        Dịch Interactive (word-by-word) - CHỈ hỗ trợ Chinese
        """
        if source_lang != "Chinese":
            return self._generate_error_html(
                "Interactive mode chỉ hỗ trợ nguồn Tiếng Trung"
            )
